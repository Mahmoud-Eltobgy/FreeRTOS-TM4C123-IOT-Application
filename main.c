...
char post[] = "GET /update?api_key=2UMGTZYTFHWSLH7A&field1=%d HTTP/1.1\r\nHost:api.thingspeak.comr\n\r\n";
...
void vTaskReadTemp(void * pv)
	{
		...
		temp=(temp*1.22*0.66)/(10);			//convert adc value to temperature
		...
		while(1){
			...
			if(status == pdTRUE)
				...
				//upload this value to the cloud
									...
									sprintf((char*)Buffer, post,tempValue )
									
									if(tempValue >= thresholdTemp)
									{
										
									DelayMs((thresholdTime));//busy wait
...
//						GPIO_PORTF_DATA_R &=~(1<<2);//turn off
  ...
  ESP8266_GetVersionNumber();
	...
	void vTaskSystemStart(void *pv)
	{
		...
		printf("Ok we will take care of this,relax....\n\r");
		vTaskSuspend(NULL);
		}
